var mobiworks = window.mobiworks || {};

(function () {
    function databindRender (node, expr, scope) {
        var parts = expr.split(".");
        var current = scope;
        for ( var j = 0; j < parts.length; j++) {
            current = current[parts[j]];
        }
        current.list(null, function (array) {
            var template;
            if (node.data("template")) {
                template = node.data("template");
            } else {
                template = node.html();
                node.data("template", node.html());
            }
            var iteratorItem = node.attr("item");
            node.empty();
            for ( var j = 0; j < array.length; j++) {
                (function () {
                    var subSource = Object.create(scope);
                    node.append(template);
                    subSource[iteratorItem] = array[j];
                    var n = node.children().last();
                    n.scope(subSource);
                    n.databind();
                }());
            }
        });
        // setTimeout(scrollTo, 0, 0, 1);
    }

    function setScopeExpr (scope, expr, val) {
        var parts = expr.split(".");
        var current = scope;
        for ( var j = 0; j < parts.length - 1; j++) {
            current = current[parts[j]];
        }
        var property = parts[parts.length - 1];
        current[property] = val;
    }

    function isOrphan (node) {
        return node.parents("html").length === 0;
    }

    jQuery.fn.databind = function (rebinding) {
        // TODO: maybe do a cleanData call on all children to avoid memory
        // leaks?
        var that = this;
        for ( var idx = 0; idx < this.length; idx++) {
            (function () {
                var node = that.eq(idx);
                if (node.attr("databind")) {
                    var scope = node.scope();
                    var expr = node.attr("databind");
                    var parts = expr.split(".");
                    var current = scope;
                    // node.removeAttr("databind");

                    // console.log(expr);
                    for ( var j = 0; j < parts.length - 1; j++) {
                        current = current[parts[j]];
                    }
                    var property = parts[parts.length - 1];
                    if (!current || current[property] === undefined) {
                        console.log("Not found: " + expr);
                    }
                    var tag = node[0].tagName;
                    if (current[property].addEventListener) { // it's an
                        // observable
                        databindRender(node, expr, scope);
                        // Add listeners
                        current[property].addEventListener( [ "add", "remove", "addAll" ], function () {
                            databindRender(node, expr, scope);
                        });
                    } else {
                        // node.removeAttr("databind");
                        if ($.inArray(tag, [ "INPUT", "SELECT" ]) != -1) {
                            switch (node.attr("type")) {
                            case "text":
                                if (current.addEventListener) {
                                    current.addEventListener("set", function (_, _, prop, val) {
                                        // TODO add isOrphan check
                                            if (prop === property) {
                                                node.val(val);
                                            }
                                        });
                                }
                                node.keyup(function () { // keyup or
                                            // onchange?
                                            setScopeExpr(scope, expr, node.val());
                                        });
                                node.val(current[property]);
                                break;
                            case "checkbox":
                                if (current.addEventListener) {
                                    console.log("adding current")
                                    console.log(current)
                                    current.addEventListener("set", function (_, _, prop, val) {
                                        if (prop === property) {
                                            console.log("Property: " + property)
                                            node.attr("checked", val);
                                        }
                                    });
                                }
                                node.attr("checked", current[property]);
                                node.change(function () {

                                    setScopeExpr(scope, expr, !!node.attr("checked"));
                                });
                                break;
                            }
                        } else if (tag == "IMG") {
                            node.attr("src", current[property]);
                        } else {
                            if (current.addEventListener) {
                                current.addEventListener("set", function (_, _, prop, val) {
                                    if (prop === property) {
                                        node.text(val);
                                    }
                                });
                            }
                            node.text(current[property]);
                        }
                    }
                } else if (!node.hasClass("template") && (rebinding || !node.hasClass("templatecall"))) {
                    // console.log("calling databind recursively")
                    node.children().databind(rebinding);
                }
                node.enableExtraEvents();
            }());
        }
    };
}());
